...
def train_loop(model_in, train_dl, test_dl, epochs, uuid_, k_folds,lr, weight_decay,filein):
    '''Function containing the neural net model training loop'''
    optimizer = optim.AdamW(model_in.parameters(), lr=lr, weight_decay=weight_decay)
...
def train_camull( k_folds=5, model=None, epochs=40):
    ...
    for k_ind in range(k_folds):
        ...
        train_loop(model, train_data, test_data, epochs, uuid_, k_folds=k_ind+1,lr=lr, weight_decay=weight_decay,filein=filein)
